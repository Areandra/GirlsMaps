...
import alt from "../../assets/Service2.png";

const DeskripsiModal = ({ visible }) => {
  ...
  return (
    ...
    >
      <img style={styles.img} src={currentPin?.urlImage || alt}></img>
      <div style={styles.contentContainer}>
          ...
          currentPin?.product[0].merek !== "" ? (
            <>
              {currentPin?.product.map((i) => (
                ...
                >
                  <p style={{ fontSize: 14, color: ColorPallate.primary }}>
                    {i.merek} :
                  </p>
                  <div
                    style={{
        ...
        <div style={styles.line}></div>
        <h1 style={styles.tittleText}>Review</h1>
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {dataContoh?.lenght !== 0 ? (
            <>
              {dataContoh.map((i) => (
                ...
                >
                  ...
                  >
                    {Array.from({ length: i.rating }, (__, idx) => (
                      <BsFillStarFill key={idx} size={10} color="gold" />
                    ))}
                    <p style={styles.lebelInfo}>
                      {() => {
                        const now = new Date();
                        const commentTime = new Date(i.createdAt);

                        const second = now.getTime() - commentTime.getTime();
                        const ago = 0;
                        return ago;
                      }}
                    </p>
                  </div>
                  <p style={styles.lebelInfo}>{i.comment}</p>
                </div>
              ))}
            </>
          ) : (
            <p style={styles.lebelInfo}>
              Information of This Store Reviwer Currently not Available
            </p>
          )}
        </div>
      </div>
